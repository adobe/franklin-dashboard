import { Flex, DateRangePicker, TextField, Form, ButtonGroup, Button, Divider, NumberField, Text} from '@adobe/react-spectrum';
...
export function Filter({ hasCheckpoint, dataEndpoint, apiEndpoint, data, setter, dataFlag, flagSetter }){
    ...
    const getQuery = (cfg = {}) => {
        const { url, domainkey, startdate, enddate, hostname, limit } = cfg;
        queryRequest(dataEndpoint, apiEndpoint, {domainkey, url, startdate, enddate, hostname, limit});
      };
    ...
    const onSubmit = (e) => {
        ...
        const { start, end, ckpt, inputUrl, domainkey, limit } = data;

        const currentpage = new URL(window.location.href);
        const params = currentpage.searchParams;
        const url = inputUrl ? inputUrl : params.get('url');
        const hostname = url ? new URL(url.startsWith('https://') ? url : "https://"+url).hostname : ''
        const key = domainkey ? domainkey : params.get('domainkey') ? params.get('domainkey') : hostname ? localStorage.getItem(hostname) : '';
        ...
        <>
            <Flex direction="column" alignItems="center" height="100%" id='filter' rowGap={'size-250'}>
                ...
                <Form onSubmit={onSubmit} method='get'>
                    ...
                    <NumberField name='limit' label="Limit"  defaultValue={10} minValue={10} /> 
                    <ButtonGroup>
